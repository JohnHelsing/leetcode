...
// 标识符位于栈顶的函数是当前正在执行的函数 。每当一个函数开始或者结束时，将会记录一条日志，包括函数标识符、是开始还是结束、以及相应的时间戳。
//
...
//
// 
//输入：n = 1, logs = ["0:start:0","0:start:2","0:end:5","0:start:6","0:end:6","0:e
//nd:7"]
//输出：[8]
//解释：
...
//
// 
//输入：n = 2, logs = ["0:start:0","0:start:2","0:end:5","1:start:6","1:end:6","0:e
//nd:7"]
//输出：[7,1]
//解释：
//函数 0 在时间戳 0 的起始开始执行，执行 2 个单位时间，并递归调用它自身。
...
//
// 
//输入：n = 2, logs = ["0:start:0","0:start:2","0:end:5","1:start:7","1:end:7","0:e
//nd:8"]
//输出：[8,1]
// 
//
...
public class Q0636_ExclusiveTimeOfFunctions {
    ...
    class Solution {
        ...
        public int[] stack(int n, List<String> logs) {
