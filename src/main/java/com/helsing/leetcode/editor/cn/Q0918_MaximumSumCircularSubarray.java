...
public class Q0918_MaximumSumCircularSubarray {
    ...
    class Solution {
        public int maxSubarraySumCircular(int[] nums) {
            // 单调队列
            return queueMonotone(nums);
        }

        public int queueMonotone(int[] nums) {
            int n = nums.length;

            // Compute P[j] = B[0] + B[1] + ... + B[j-1]
            // for fixed array B = A+A
            int[] circle = new int[2 * n + 1];
            for (int i = 0; i < 2 * n; ++i) {
                circle[i + 1] = circle[i] + nums[i % n];
            }
            ...
            for (int j = 1; j <= 2 * n; ++j) {
                ...
                ans = Math.max(ans, circle[j] - circle[deque.peekFirst()]);

                // Remove any i1's with P[i2] <= P[i1].
                while (!deque.isEmpty() && circle[j] <= circle[deque.peekLast()]) {
                    deque.pollLast();
                }
