...
public class Q1601_MaximumNumberOfAchievableTransferRequests {
    ...
    class Solution {
        ...
        //回溯，枚举每个组合，回溯判断的请求列表从长到短，易获取到最大的请求数目
        //获取到相对大的请求数目后就可以据此对剩下的分支进行剪枝判断
        ...
        public int maximumRequests(int n, int[][] requests) {
            ...
            for (int i = 0; i < requestNum; i++) {
                if (requestNum - i < maxRequest) break;
                dfs(requests, i, 1);
            }
            return maxRequest;
        }
        ...
        public void dfs(int[][] requests, int index, int request) {
            ...
            for (int i = index + 1; i < requestNum; i++) {
                if (requestNum - i + request < maxRequest) break;
