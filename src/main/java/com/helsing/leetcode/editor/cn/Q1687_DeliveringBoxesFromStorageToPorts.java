...
// 对于在卡车上的箱子，我们需要 按顺序 处理它们，卡车会通过 一趟行程 将最前面的箱子送到目的地码头并卸
// 货。如果卡车已经在对应的码头，那么不需要 额外行程
//，箱子也会立马被卸货。 
...
//输出：4
//解释：最优策略如下：
//- 卡车将所有箱子装上车，到达码头 1 ，然后去码头 2 ，然后再回到码头 1 ，最后回到仓库，
// 总共需要 4 趟行程。
//所以总行程数为 4 。
//注意到第一个和第三个箱子不能同时被卸货，因为箱子需要按顺序处理
// （也就是第二个箱子需要先被送到码头 2 ，然后才能处理第三个箱子）。
// 
//
// 示例 2： 
//
...
// 
// Related Topics 线段树 队列 数组 动态规划 单调队列 堆（优先队列） 👍 28 👎 0
...
public class Q1687_DeliveringBoxesFromStorageToPorts {
    ...
    class Solution {
        public int boxDelivering(int[][] boxes, int portsCount, int maxBoxes, int maxWeight) {
            // 动态规划 单调队列
            return dpQueMonotone(boxes, portsCount, maxBoxes, maxWeight);
        }

        public int dpQueMonotone(int[][] boxes, int portsCount, int maxBoxes, int maxWeight) {
            int n = boxes.length;
            int[] p = new int[n+1];
            int[] w = new int[n+1];
            int[] neg = new int[n+1];
